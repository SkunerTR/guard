...
client.on('ready', async () => {
 ...
 client.user.setActivity(`w!yardım`, { type:'WATCHING' })
...
let kanal = await db.fetch(`antiraidK_${member.guild.id}`)== "anti-raid-aç"
  ...
  if (member.user.bot === true) {
    ...
    darknesyt.send(darknesguardv2);
      ...
      .setColor("RANDOM")
      .setThumbnail(member.user.avatarURL())
      .setDescription("**" + member.user.tag +"**" + " (" + member.id+ ") " + "adlı bot sunucuya eklendi ve Attım eğer izin vermek istiyorsanız **" + "w!bot-izni ver <botid>**")
...
const voiceChannel = "814512960109150218"
